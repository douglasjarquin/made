...
func TestRun_PRBodyRendersPipelineSummaryAndKeepsAttestation(t *testing.T) {
	logPath := filepath.Join(t.TempDir(), "invocations.log")
	c := newClient(t, []string{"FAKE_GH_PR_URL=https://github.com/example/repo/pull/11"}, logPath)

	summary := "> Generated by [`made`](https://github.com/douglasjarquin/made)\n\n| Stage | Status | Notes |\n|---|:---:|---|\n| Intent | ✅ Passed | |\n"
	_, err := pr.Run(context.Background(), c, pr.Options{
		Title:           "made: automated change",
		Base:            "main",
		Head:            "made/run-777",
		EvidenceRef:     "made-evidence:run-777",
		RunID:           "run-777",
...
func TestRun_PRBodyWithoutSummaryStillCarriesAttestation(t *testing.T) {
	logPath := filepath.Join(t.TempDir(), "invocations.log")
	c := newClient(t, []string{"FAKE_GH_PR_URL=https://github.com/example/repo/pull/12"}, logPath)

	_, err := pr.Run(context.Background(), c, pr.Options{
		Title:       "made: automated change",
		Base:        "main",
		Head:        "made/run-888",
		EvidenceRef: "made-evidence:run-888",
		RunID:       "run-888",
	})
	if err != nil {
		t.Fatalf("Run: %v", err)
	...
	log := string(data)
	for _, want := range []string{"## Pipeline", "Run-ID: run-888", "Protocol-Version: "} {
		if !strings.Contains(log, want) {
			t.Fatalf("expected PR body to still contain %q, log:\n%s", want, log)
		}
	}
}
