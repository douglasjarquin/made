...
func VerifyCommitted(path string) error {
	...
	want := Markdown()
	if string(got) != want {
		return fmt.Errorf("skill: %s has drifted from internal/skill's generated output; run `make skill` (go run ./cmd/genskill) and commit the result", path)
	}
	return nil
}
