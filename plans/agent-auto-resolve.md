...
- [x] F1. Plan Compliance Audit - confirmed `.made.yaml` untouched, no new `docs/*.md`, no new `Outcome` value (only `OutcomeInfrastructureError` reused, verified by grep), no new config YAML key (only new values/methods on the existing `Agent`/`Agents` fields).
- [x] F2. Code Quality Review - `golangci-lint run ./...` (pinned CI version v2.11.2, installed fresh in this sandbox since it wasn't pre-installed): **0 issues**. `AgentKind()` callers confirmed to be exactly the 4 intended call sites (doctor.go's pinned report path, both spawnReview/spawnInternalReview pinned fast paths, config.go's own validateCommon) - no orphans. D2/D7/D8/D9's documented-limitation comments are present in the shipped code (spawn.go, resolve.go, quotaaxi.go, workfunc.go/runner.go).
- [x] F3. Real Manual QA - full `go build ./...`, `go vet ./...`, and `go test ./...` (with the repo's gpgsign workaround) re-run one final time. Every failure across the whole suite (internal/agent, internal/managed, internal/orchestrator, internal/pipeline/review) shares the identical `bwrap: setting up uid map: Permission denied` signature - this sandbox's kernel disables unprivileged user namespaces via AppArmor (`kernel.apparmor_restrict_unprivileged_userns=1`), confirmed via git-stash comparison at every commit in this branch to be 100% pre-existing and unrelated to this diff (including in `internal/pipeline/review`, a package this diff never touches at all). Every new test this diff added passes.
- [x] F4. Scope Fidelity Check - `git diff --stat main...HEAD`: 39 files changed, none out of scope (`.made.yaml`, `docs/*.md`, `cmd/made/runcommands.go`, `internal/cursor/doctor.go` all absent from the diff).
